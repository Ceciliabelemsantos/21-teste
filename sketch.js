...
function draw() 
{
  ...
  if (gameState==="play")
    {
      ...
      if (obst1Group.collided(oito)) {
        obst1Group.destroyEach();
        score=score+5;
      }
      if (obst3Group.collided(oito)) {
        obst2Group.destroyEach();
        score=score+10;
      }
      if (obst4Group.collided(oito)) {
        obst4Group.destroyEach();
        score=score+5;
      }
      if (oito.collided(obst2Group))
      {
       gameState="end";
      }
    }
  ...
  if (gameState==="end")
  {
    ...
    recomeçar.visible=true;
     fundo.velocityY = 0;
     oito.velocityY = 0;
     obstGroup1.setVelocityXEach(0);
     obstGroup2.setVelocityXEach(0);
     obstGroup3.setVelocityXEach(0);
     obstGroup4.setVelocityXEach(0);

     obstGroup1.setLifetimeEach(-1);
     obstGroup2.setLifetimeEach(-1);
     obstGroup3.setLifetimeEach(-1);
     obstGroup4.setLifetimeEach(-1);
